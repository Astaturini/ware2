...
As of v0.7.2, every benchmark run is recorded through ExperimentRunner and
writes the standard artifacts (config.json, timeseries.csv, events.csv,
summary.json) to data/runs/<run_id>/.
"""
...
def run_benchmark(args: argparse.Namespace) -> None:
    ...
    if args.spawn == "legacy":
        ...
        def sim_factory(cfg: ExperimentConfig) -> Simulation:
            warehouse = build_warehouse()
            robots = create_legacy_robots(cfg.num_robots)
            battery_config = BatteryConfig(
                capacity=cfg.battery_capacity,
                empty_move_energy=cfg.empty_move_energy,
                loaded_move_energy=cfg.loaded_move_energy,
                critical_battery=cfg.critical_battery,
                opportunistic_charge_threshold=cfg.opportunistic_charge_threshold,
                idle_ticks_before_opportunistic_charge=cfg.idle_ticks_before_opportunistic_charge,
                safety_margin=cfg.battery_safety_margin,
                empty_battery_recovery_ticks=cfg.empty_battery_recovery_ticks,
            )
            charging_config = ChargingConfig(
                capacity=cfg.charger_capacity,
                charge_duration_ticks=cfg.charge_duration_ticks,
            )
            failure_config = FailureConfig(
                enabled=cfg.failure_enabled,
                mtbf_ticks=cfg.mtbf_ticks,
                mttr_ticks=cfg.mttr_ticks,
                seed=cfg.seed,
                relocate_to_maintenance=True,
            )
            metrics = Metrics()
            path_planner = create_path_planner_from_name(cfg.path_planner)
            scheduler = create_scheduler_from_name(cfg.scheduler, path_planner)
            sim = Simulation(
                warehouse=warehouse,
                robots=robots,
                tasks=[],
                ...
                scheduler=scheduler,
                task_generator=TaskGenerator(warehouse=warehouse, seed=cfg.seed),
                metrics=metrics,
                blocked_replan_seconds=cfg.blocked_replan_seconds,
                replan_cooldown_ticks=cfg.replan_cooldown_ticks,
            ...
            if hasattr(sim, "set_conflict_manager"):
                sim.set_conflict_manager(
                    create_conflict_manager_from_name(cfg.conflict_manager, sim)
                )
            return sim
    else:
        sim_factory = create_simulation_from_config # type: ignore 

    runner = ExperimentRunner()
    start_time = time.perf_counter()
    run_id = runner.start(config, sim_factory)
    print(f"Run id: {run_id}")
    print(f"Recording artifacts to: {runner.run_dir}")
    ...
    stop_reason = runner.stop_reason or "unknown"
