...
### v0.8.1 adds Optimization Layer on top of v0.8.0 Uncertainty and Risk Analysis

New responsibilities added in v0.8.1:

Business Optimization:
- Optuna-based single-objective optimization in decision/optimizer.py.
- Minimizes or maximizes a selected operational objective.
- Supports hard constraint checks such as p95_cycle_time_max and sla_compliance_rate_min.
- Explicitly reports infeasible search spaces instead of silently returning bad configurations.

Multi-Objective Optimization:
- NSGA-II based Pareto front generation in decision/multiobjective.py.
- Supports multiple objectives such as minimizing cost_per_task while minimizing p95_cycle_time.
...
- Multi-replica validation of candidate configurations in decision/robustness.py.
- Calculates constraint_pass_probability for each candidate.
- Recommends a candidate only if its constraint pass probability meets min_pass_probability.
- Intended to verify Monte Carlo, sensitivity, optimizer, and Pareto candidates before deployment.

No simulation core changes were introduced in v0.8.1.
No ExperimentConfig schema changes were introduced in v0.8.1.
No frontend changes were introduced in v0.8.1.
All new modules are decision-layer CLI modules and follow the No UI First rule.

### v0.8.0 adds Uncertainty and Risk Analysis on top of v0.7.2 Spatial Diagnostics

New responsibilities added in v0.8.0:

Monte Carlo Runner:
- Executes N identical scenarios with varying seeds.
- Calculates confidence intervals, p5/p95 KPIs, and probability of SLA violation.
- Writes derived Monte Carlo artifacts to data/monte_carlo/<mc_id>/.
- Each Monte Carlo trial still writes a normal immutable run artifact to data/runs/<run_id>/.

Sensitivity Analysis:
- One-at-a-time parameter variation around a baseline configuration.
- Outputs tornado data showing parameter impact on cost_per_task and p95_cycle_time.
- Writes derived sensitivity artifacts to data/sensitivity/<study_id>/.
- Each sensitivity trial still writes a normal immutable run artifact to data/runs/<run_id>/.

No simulation core changes were introduced in v0.8.0.
No ExperimentConfig schema changes were introduced in v0.8.0.
No frontend changes were introduced in v0.8.0.

### v0.7.2 adds Spatial Diagnostics on top of the unchanged v0.7.1 Decision and Realistic Scenario layers

Spatial bottleneck analysis:
- Blockage onset coordinates are recorded in events.csv.
- Per-cell bottleneck statistics are computed from saved artifacts only.

Recorded benchmarks:
- benchmark.py records every run through ExperimentRunner into data/runs/<run_id>/.

Single-run engineering reports:
- decision/report.py can report one run without a study.
- Single-run reports are written to data/reports/<run_id>_report.md.
- Run artifacts remain immutable.

### v0.7.1 adds a Decision Layer and Realistic Scenario layer on top of the unchanged v0.6.0 algorithm plugin layer and unchanged v0.5.0 experimentation, recording, storage, and analysis infrastructure
...
- Study outputs are written to data/studies/<study_id>/.

Engineering reporting:
- Markdown study reports generated from saved study results.

Dynamic demand profiles:
- Legacy synthetic demand preserved.
- uniform, rate_schedule, and csv_orders demand modes added.

Configurable layout presets:
- Default warehouse preserved.
- JSON layout definitions and preset layouts added.
...
Simulation:
- simulation/
- v0.4 core with v0.6 plugin injection points.

Experiment configuration:
- experiment/config.py
- authoritative run parameters.
...
Setup -> Run live or Fast headless -> Results -> Experiments -> Visualization -> Analysis -> Compare
...
Simulation constructor now accepts keyword-only v0.4 arguments: battery_config, charging_config, failure_config, and seed.
Simulation now owns charging-station state, maintenance-cell scanning, failure RNG, dynamic resume-location tracking, and congestion snapshots.
...
Added taskless routing helper _route_robot_to_cell() for charger and maintenance movement.

### Data Models

Robot gained RobotMode and v0.4 battery/charging/failure fields.
Task gained LoadState, INTERRUPTED status, interruption count, last-assigned robot, and resume-location fields.
...
Frontend supports Interrupted tasks, robot mode/battery tooltips, and v0.4 dashboard metrics.
...
Added experiment/ package: config.py, factory.py, recorder.py, runner.py.
Added analysis/ package: loader.py, metrics.py, web.py.
app.py uses a mutable holder = {"simulation": ..., "runner": ...}; the v0.4 closure-based single-simulation wiring is gone.
create_default_simulation() replaced by create_simulation_from_config(ExperimentConfig.default()).
Robot creation is dynamic and seeded; old hardcoded 13-position list is gone.
No robot-count cap beyond available passable cells.
ExperimentConfig gained display_name, label only, and fast_mode, pacing only; fast_mode never affects results.
New routes: experiment lifecycle, run deletion, and analysis JSON API.
Frontend rewritten: view state machine, MultiSelect component, ECharts charts, experiment delete, legend, fast-run progress view.
CSS rewritten: dark theme, flat borderless tiles, glow-only task/location highlights, square robots.
...
- Added PathPlanner protocol.
- Added BFSPathPlanner.
- Added AStarPathPlanner.
- find_shortest_path() preserved as backward-compatible wrapper delegating to BFSPathPlanner.

simulation/scheduler.py UPDATED:
- Added PriorityScheduler.
- Added FIFOScheduler.
- Added TotalCostScheduler.
- Added AuctionScheduler.
- Existing CostBasedScheduler preserved.
...
Simulation gained _conflict_manager internal state, initialized to LocalYieldConflictManager(self).
Simulation._tick() now calls self._conflict_manager.tick() before _advance_robots().
Simulation._advance_robots() now calls self._conflict_manager.allow_step() before the occupied-cell check.
Simulation._congestion_snapshot() reads self._conflict_manager.active_conflicts.
...
ExperimentConfig.__post_init__ validates path_planner against PATH_PLANNER_CHOICES.
ExperimentConfig.__post_init__ validates conflict_manager against CONFLICT_MANAGER_CHOICES.

### Factory

experiment/factory.py gained create_path_planner_from_name(name).
experiment/factory.py gained create_scheduler_from_name(name, path_planner).
experiment/factory.py gained create_conflict_manager_from_name(name, sim).
...
Choices: baseline, priority, fifo, total_cost, auction. Default: baseline.

benchmark.py gained --conflict-manager argument.
Choices: local_yield, zone_locks, priority_reservation. Default: local_yield.
...
Run view restructured: .run-layout flex container with .run-main and .run-sidebar.
Pause/Resume/Reset buttons moved to .run-controls at top of sidebar.
readExperimentConfig() includes path_planner, scheduler, and conflict_manager.
...
- Includes trade-off observations.
...
create_simulation_from_config() now builds the warehouse using decision.layout.create_warehouse_for_experiment().
create_simulation_from_config() now builds the task generator using create_task_generator().
...
MetricsRecorder writes {"x": ..., "y": ...} JSON into the details column of robot_blocked events.
...
- internal _pair_blocked_episodes().

Reads saved artifacts only.
Blockage episodes are paired locally from robot_blocked/robot_unblocked events.
Unclosed episodes are censored at the final simulation tick.
Per-cell metrics:
- blocked_events, onset count.
- blocked_ticks, episode duration attributed to the onset cell.
...
This section is driven by the run_id column in results.csv.
Studies that vary layout_preset are flagged as not comparable across runs.

python -m decision.report gained --run-id.
--run-id is mutually exclusive with --study-dir.
python -m decision.report gained --base-dir.
...
It writes config.json, timeseries.csv, events.csv, and summary.json to data/runs/<run_id>/.
...
results.csv carries run_id so each study row traces back to data/runs/<run_id>/.
...
Monte Carlo:
- Executes N identical experiment configurations with varying seeds.
...
- Optuna TPESampler.
- Constraint handling through constraints_func.
...
weight = 1.0:
Standard A*.
On a uniform 4-connected grid, returns shortest paths like BFS, but may choose a different shortest path when ties exist.

weight > 1.0:
Weighted A*.
Faster search, but paths may be suboptimal.
...
Processes tasks by (-priority, created_at, id).
Constructor accepts optional path_planner keyword argument.
Defaults to BFSPathPlanner().
...
Processes tasks by (priority, created_at, id), where lower priority value = higher urgency.
...
Processes tasks by (created_at, priority, id).
For each task, assigns the nearest feasible robot to the pickup.
...
For each task, computes estimated total cost as distance_to_pickup + distance_pickup_to_dropoff.
Assigns the robot with the lowest total cost.
Constructor accepts optional path_planner, empty_move_energy, loaded_move_energy, safety_margin, and critical_battery keyword arguments.
...
Bid cost is distance_to_pickup + distance_pickup_to_dropoff.
...
Diagnostic counters: denied_steps, locks_created, tick_calls, robots_with_paths.
...
Starvation fallback: blocked_ticks >= blocked_replan_threshold_ticks * 2.
Diagnostic counters: denied_steps, reservations_created, tick_calls, robots_with_paths.
...
Important internal state, existing:
...
Important internal state, new v0.4:
...
Important internal state, new v0.6:
simulation._path_planner — injected PathPlanner instance. Defaults to BFSPathPlanner().
simulation._conflict_manager — injected ConflictManager instance. Defaults to LocalYieldConflictManager(self).
...
reset() clears conflict manager state via self._conflict_manager.reset().
reset() clears dynamic resume locations.
reset() resets charging station state.
reset() resets failure RNG.
reset() restores robot batteries/modes.
...
_advance_robots() calls self._conflict_manager.allow_step() before the occupied-cell check.
Replanning/yielding robots are NOT gated by allow_step() so deadlock escape still works.
_prune_old_tasks() deletes completed/failed tasks whose completed_at is more than 100 ticks old.
Interrupted tasks are not pruned.
...
A valid yield step must be in bounds, not a shelf, not occupied, and not claimed by another robot's immediate next target.
...
It is not reservation-based multi-agent pathfinding.
It does not implement intersection control.
...
Starvation fallback: if blocked_ticks >= blocked_replan_threshold_ticks * 2, the robot is allowed through regardless of locks.
Failed, repairing, and charging robots do not create claims.
Diagnostic counters: denied_steps, locks_created, tick_calls, robots_with_paths.
...
Starvation fallback: if blocked_ticks >= blocked_replan_threshold_ticks * 2, the robot is allowed through regardless of reservations.
Failed, repairing, and charging robots do not create reservations.
Diagnostic counters: denied_steps, reservations_created, tick_calls, robots_with_paths.
...
Executes factorial DoE matrices.
Outputs data/studies/<study_id>/results.csv.
results.csv carries a run_id column so each row traces back to data/runs/<run_id>/.
...
## Run Identity And Storage Format, v0.5.0 / v0.7.2 / v0.8.x
...
results.csv carries a run_id column so each study row traces back to data/runs/<run_id>/.
...
Each Monte Carlo trial also writes a normal immutable run artifact under data/runs/<run_id>/.
...
Each sensitivity trial also writes a normal immutable run artifact under data/runs/<run_id>/.
...
Each optimization trial may write normal immutable run artifacts under data/runs/<run_id>/.
...
Each multi-objective trial may write normal immutable run artifacts under data/runs/<run_id>/.
...
Each robustness replication writes a normal immutable run artifact under data/runs/<run_id>/.
...
scheduler
path_planner, NEW in v0.6.0
conflict_manager, NEW in v0.6.0
...
ExperimentConfig is a frozen dataclass validated in __post_init__.
...
DEMAND_MODE_CHOICES, NEW in v0.7.1:
legacy
uniform
rate_schedule
csv_orders

LAYOUT_PRESET_CHOICES, NEW in v0.7.1:
default
high_density
one_way_aisles
...
seed is required.
num_robots >= 1.
workload stop mode requires target_tasks >= 1.
max_ticks >= 1.
tick_interval > 0.
...
Builds warehouse via decision.layout.create_warehouse_for_experiment().
Builds task generator via create_task_generator().
Builds robots, configs, path planner, scheduler.
...
RuntimeError if a run is active.
...
In-memory or reads summary.json.

Internal:
_run()
Loop while not sim._stop_event.
Skip while sim.is_paused.
Under sim._lock:
_tick()
_snapshot()
recorder.record()
...
Pacing:
fast_mode -> time.sleep(0) every 500 ticks.
normal mode -> time.sleep(sim._tick_interval).

Exceptions set stop_reason = "error: ...".

_snapshot()
Primitive extraction of metrics counters.
Per-task extraction:
id
status
assigned_robot_id
...
fixed_ticks: tick >= max_ticks.
workload: completed >= target_tasks, else tick >= max_ticks.

_finalize()
Closes recorder.
Computes and writes summary.json once.
...
Invalid ids raise AnalysisApiError.
Flask routes map AnalysisApiError to 404.
...
get_run_distributions bins clamped 10..100.
Returns:
run_id
stats
histograms:
...
create_default_simulation() is replaced by create_simulation_from_config(ExperimentConfig.default()).
...
400 on validation or spawn errors.
...
GET /api/state returns the v0.4 payload plus the v0.5 top-level experiment key described in the Flask Layer section.
...
ECharts may load from CDN or be vendored at static/vendor/echarts.min.js for offline/air-gapped hosts.
...
For production use, consider queueing, batch scheduling, or dedicated worker processes.
...
Conflict manager diagnostics could be persisted to summary.json for cross-run comparison.
...
Monte Carlo storage under data/monte_carlo/
Sensitivity storage under data/sensitivity/
Optimization storage under data/optimizations/
Multi-objective storage under data/multiobjective/
Robustness storage under data/robustness/

CLI commands for all decision modules
Constraint pass probability reporting
Explicit infeasible search space reporting
Pareto front extraction from feasible multi-objective trials
Robust candidate recommendation only when min_pass_probability is met

The simulation core remains unchanged.
The experiment artifact format remains unchanged.
The analysis layer remains artifact-driven.
The frontend remains unchanged for v0.8.x.