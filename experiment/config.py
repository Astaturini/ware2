...
class ExperimentConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.stop_mode not in {"fixed_ticks", "workload"}:
            raise ValueError("stop_mode must be either 'fixed_ticks' or 'workload'.")
        if self.stop_mode == "workload":
            if self.target_tasks is None or self.target_tasks < 1:
                raise ValueError("target_tasks must be a positive integer in workload mode.")
        if self.max_ticks < 1:
        ...
        if self.failure_enabled and self.mtbf_ticks <= 0:
            raise ValueError("mtbf_ticks must be > 0 when failure_enabled is true.")
