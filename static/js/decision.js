...
function decisionTableHtml(columns, rows, options = {}) {
  if (!rows.length) {
    return `<div class="decision-status">${escapeHtml(options.emptyMessage || "No rows.")}</div>`;
  }
...
function renderRowsTable(container, rows, options = {}) {
  ...
  if (!prepared.length) {
    setDecisionEmpty(container, options.emptyMessage || "No rows.");
    return;
  }
  ...
  async function load() {
    ...
    try {
      ...
      if (!columns && rows.length) {
        columns = deriveColumns(rows[0], options.preferredKeys || []);
      }
      ...
      container.innerHTML =
        decisionTableHtml(columns, rows, options) +
        paginationControls(page, pageSize, totalRows);

      container.onclick = event => {
        if (event.target.closest(".pg-prev:not([disabled])")) {
          page = Math.max(1, page - 1);
          load();
          return;
        }

        if (event.target.closest(".pg-next:not([disabled])")) {
      ...
      setDecisionError(container, error.message);
    }
  }
...
function probabilityBarHtml(value, threshold) {
  ...
  const color =
    Number.isFinite(num) && threshold != null && num >= threshold
      ? "var(--success)"
      : "var(--error)";
  ...
  return `
    <div style="min-width:120px;">
      ...
      <div style="font-size:11px; color:var(--text-secondary); margin-top:2px;">
        ${escapeHtml(formatProbability(value))}
      </div>
    </div>
  `;
}
...
const DEFAULT_EXAMPLE_COST_CONFIG = Object.freeze({
...
function ensureRunSelectValue(selectId, runId) {
  const selectEl = document.getElementById(selectId);
  if (!selectEl || !runId) return;

  const exists = Array.from(selectEl.options).some(option => option.value === runId);

  if (!exists) {
    selectEl.add(new Option(runId, runId));
  }

  selectEl.value = runId;
}
...
function buildSensitivityFactorValues(runConfig) {
  const cfg = runConfig || {};
  const numRobots = Math.max(1, Math.floor(Number(cfg.num_robots || 6)));

  let values = uniqueSortedInts([
    Math.max(1, numRobots - 1),
    numRobots,
    numRobots + 1
  ]);

  if (values.length < 2) {
    values = uniqueSortedInts([numRobots, numRobots + 1]);
  }

  return values;
}

function buildRunLinkedSearchSpace(runConfig) {
  const cfg = runConfig || {};

  const numRobots = Math.max(1, Math.floor(Number(cfg.num_robots || 6)));
  const chargerCapacity = Math.max(1, Math.floor(Number(cfg.charger_capacity || 4)));

  const robotLow = Math.max(1, numRobots - 2);
  const robotHigh = Math.max(robotLow + 1, numRobots + 2);

  const chargerLow = Math.max(1, chargerCapacity - 2);
  const chargerHigh = Math.max(chargerLow + 1, chargerCapacity + 2);
...
function buildRunLinkedConfig(module, runId, runSummary) {
  if (!runId) {
    throw new Error("Run id is required.");
  }

  const cfg = runSummary && runSummary.config ? runSummary.config : {};
  ...
  switch (module) {
    ...
    case "sensitivity":
      return {
        ...
        factors: [
          {
            name: "num_robots",
            values: buildSensitivityFactorValues(cfg)
          }
        ]
      };
    ...
    case "optimizer":
      return {
        ...
        base_seed: 42,
        constraints: {},
        search_space: buildRunLinkedSearchSpace(cfg)
      };
    ...
    default:
      throw new Error(`Unsupported run-linked decision module: ${module}`);
  }
}

async function fetchRunSummarySafe(runId) {
  const url = `/api/runs/${encodeURIComponent(runId)}/summary`;

  try {
    if (typeof decisionFetchJson === "function") {
      return await decisionFetchJson(url);
    }
    ...
    if (!res.ok) {
      throw new Error(data.error || "Failed to fetch run summary.");
    }

    return data;
  } catch (error) {
    console.error("Could not fetch run summary:", error);
    return null;
  }
}

function showRunActionError(message) {
  console.error(message);
  window.alert(message);
}

async function prepareDecisionRunSelect(selectId, runId) {
  if (typeof populateRunSelects === "function") {
    try {
      await populateRunSelects();
    } catch (error) {
      console.error("Could not populate run selects:", error);
    }
  }

  ensureRunSelectValue(selectId, runId);
}

async function goToCostKpisForRun(runId) {
  if (!runId) {
    showRunActionError("No run is selected.");
    return;
  }

  showView("decision");
  showDecisionSubview("cost");

  await prepareDecisionRunSelect("cost-run-select", runId);

  const costEditorEl = document.getElementById("cost-config-editor");

  if (costEditorEl && !costEditorEl.value.trim()) {
    costEditorEl.value = JSON.stringify(DEFAULT_EXAMPLE_COST_CONFIG, null, 2);
  }

  if (typeof generateCostKpis === "function") {
    generateCostKpis();
  }
}

async function goToSpatialForRun(runId) {
  if (!runId) {
    showRunActionError("No run is selected.");
    return;
  }
  ...
  showView("decision");
  showDecisionSubview("spatial");

  await prepareDecisionRunSelect("spatial-run-select", runId);

  const topEl = document.getElementById("spatial-top");
  const byEl = document.getElementById("spatial-by");

  if (topEl) topEl.value = "10";
  if (byEl) byEl.value = "blocked_ticks";

  if (typeof generateSpatial === "function") {
    generateSpatial();
  }
}

async function openRunLinkedJobModal(module, runId) {
  if (!runId) {
    showRunActionError("No run is selected.");
    return;
  }
  ...
  try {
    const runSummary = module === "report"
      ? null
      : await fetchRunSummarySafe(runId);

    const config = buildRunLinkedConfig(module, runId, runSummary);
    openJobModal(module, config);
  } catch (error) {
    console.error(error);
    showRunActionError(
      error && error.message
        ? error.message
        : "Could not prepare decision job from run."
    );
  }
}

async function handleRunLinkedAction(button) {
  if (!button) return;
  ...
  const runId =
    ...
    (typeof activeRunId !== "undefined" ? activeRunId : null);
  ...
  switch (action) {
    case "cost":
      await goToCostKpisForRun(runId);
      return;

    case "spatial":
      await goToSpatialForRun(runId);
      return;
    ...
    case "optimizer":
    case "multiobjective":
      await openRunLinkedJobModal(action, runId);
      return;

    default:
      showRunActionError(`Unsupported run action: ${action}`);
  }
}
...
function showDecisionSubview(name) {
  ...
  document.querySelectorAll(".decision-subview").forEach(el => {
    el.hidden = true;
  });
...
function refreshDecision() {
  showDecisionSubview(decisionSubview);
}
...
async function loadDecisionOverview() {
  ...
  try {
    ...
    const cards = [
      ["Studies", data.studies, "studies"],
      ["Reports", data.reports, "reports"],
      ["Monte Carlo", data.monte_carlo, "monte-carlo"],
      ["Sensitivity", data.sensitivity, "sensitivity"],
      ["Optimization", data.optimizations, "optimization"],
      ["Multi-Objective", data.multiobjective, "multiobjective"],
      ["Robustness", data.robustness, "robustness"],
      ["Jobs", data.jobs, "jobs"]
    ];
    ...
    setDecisionError(el, error.message);
  }
}
...
async function loadDecisionStudies() {
  ...
  try {
    ...
    if (!studies.length) {
      setDecisionEmpty(el, "No studies found.");
      return;
    }
    ...
    renderDecisionTable(
      ...
      [
        ...
        },
        {
          key: "action",
          label: "Action",
          render: row => `<button data-open-study="${escapeHtml(row.study_id)}">Open</button>`
        }
      ],
      studies
    );
...
async function loadStudyDetail(studyId) {
  ...
  try {
    const detailData = await decisionFetchJson(
      `/api/decision/studies/${encodeURIComponent(studyId)}`
    );

    const resultsData = await decisionFetchJson(
      `/api/decision/studies/${encodeURIComponent(studyId)}/results?page=1&page_size=50`
    );
    ...
    if (columns.includes("run_id")) {
      columns = ["run_id", ...columns.filter(column => column !== "run_id")];
    }

    el.innerHTML = `
      <div class="decision-toolbar">
        <button id="study-back-btn">Back to studies</button>
      </div>
      ...
      <details>
        <summary>Study config</summary>
        <pre class="json-preview">${escapeHtml(JSON.stringify(detailData.config || {}, null, 2))}</pre>
      </details>

      <h3>Results</h3>
      <div id="study-results-table"></div>
    `;
    ...
    renderDecisionTable(
      ...
      columns.map(key => ({
        ...
        render:
          key === "run_id"
            ? row => `<button data-open-run="${escapeHtml(row.run_id)}">${escapeHtml(row.run_id)}</button>`
            : undefined
      })),
      rows,
      "No study result rows."
    );
    ...
    el.onclick = event => {
      if (event.target.closest("#study-back-btn")) {
        loadDecisionStudies();
      }
    };
  } catch (error) {
...
async function loadDecisionReports() {
  ...
  try {
    ...
    if (!reports.length) {
      setDecisionEmpty(el, "No reports found.");
      return;
    }
...
async function openReport(reportId) {
  ...
  try {
    const data = await decisionFetchJson(
      `/api/decision/reports/${encodeURIComponent(reportId)}`
    );
    ...
    el.onclick = event => {
      if (event.target.closest("#report-back-btn")) {
        loadDecisionReports();
      }
    };
  } catch (error) {
...
async function generateSpatial() {
  ...
  if (!runId) {
    setDecisionError(el, "Select a run.");
    return;
  }
  ...
  try {
    const data = await decisionFetchJson(
      `/api/runs/${encodeURIComponent(runId)}/spatial?top=${encodeURIComponent(top)}&by=${encodeURIComponent(by)}`
    );

    const warning =
      data.coverage === 0 || data.coverage === 0.0
    ...
    el.innerHTML = `
      ...
      <div class="decision-status warning">
        Spatial coordinates are approximate.
      </div>
    ...
    renderDecisionTable(
      el.querySelector("#spatial-table"),
      [
        { key: "x", label: "x" },
        { key: "y", label: "y" },
        { key: "blocked_events", label: "blocked_events" },
        { key: "blocked_ticks", label: "blocked_ticks" }
      ],
      data.top_cells || [],
      "No bottleneck cells returned."
    );
  } catch (error) {
    setDecisionError(el, error.message);
  }
...
async function generateCostKpis() {
  ...
  try {
    ...
    if (
      costConfig === null ||
      typeof costConfig !== "object" ||
      Array.isArray(costConfig)
    ) {
      throw new Error("Cost config must be a JSON object.");
    }
  } catch (error) {
    setDecisionError(el, `Invalid cost config JSON: ${error.message}`);
    return;
  }
  ...
  try {
    const url =
      `/api/runs/${encodeURIComponent(runId)}/cost-kpis?cost_config=` +
      encodeURIComponent(JSON.stringify(costConfig));

    const data = await decisionFetchJson(url);

    const currency = data.currency || "";
    const breakdownRows = Object.entries(data.cost_breakdown || {}).map(
    ...
    renderDecisionTable(
      el.querySelector("#cost-breakdown-table"),
      [
        { key: "item", label: "Item" },
        {
          key: "value",
          label: `Cost (${currency || "USD"})`,
          render: row => escapeHtml(formatMoney(row.value, currency))
        }
      ],
      breakdownRows,
...
async function loadDecisionMonteCarlo() {
  ...
  try {
    ...
    if (!items.length) {
      setDecisionEmpty(el, "No Monte Carlo studies found.");
      return;
    }
    ...
    renderDecisionTable(
      el,
      [
        { key: "mc_id", label: "Monte Carlo ID" },
        { key: "generated_at", label: "Generated" },
        { key: "n_runs_requested", label: "Requested" },
        { key: "successful_runs", label: "Successful" },
        { key: "failed_runs", label: "Failed" },
        { key: "effective_sla_target_ticks", label: "SLA Target" },
        {
          key: "action",
          label: "Action",
          render: row => `<button data-open-mc="${escapeHtml(row.mc_id)}">Open</button>`
        }
      ],
      items
    );
...
async function openMonteCarloDetail(mcId) {
  ...
  try {
    const data = await decisionFetchJson(
      `/api/decision/monte-carlo/${encodeURIComponent(mcId)}`
    );
    ...
    el.innerHTML = `
      <div class="decision-toolbar">
        <button data-back-mc>Back to Monte Carlo studies</button>
      </div>
      ...
      <h3>Trial Results</h3>
      <div class="mc-results-table"></div>
    `;

    if (toNumber(summary.failed_runs, 0) > 0) {
      renderErrorsTable(el.querySelector(".mc-errors-table"), errors);
    }
    ...
    renderDecisionTable(
      ...
      [
        ...
        { key: "p95", label: "p95" },
        { key: "max", label: "max" },
        { key: "mean_ci95_low", label: "ci95_low" },
        { key: "mean_ci95_high", label: "ci95_high" }
      ],
      kpiRows,
      "No aggregate KPIs."
    );
    ...
    renderPaginatedDynamicTable(
      ...
      (page, pageSize) =>
        `/api/decision/monte-carlo/${encodeURIComponent(mcId)}/results?page=${page}&page_size=${pageSize}`,
      {
        flatten: true,
        linkRunId: true,
        preferredKeys: [
...
async function loadDecisionSensitivity() {
  ...
  try {
    ...
    if (!items.length) {
      setDecisionEmpty(el, "No sensitivity studies found.");
      return;
    }
    ...
    renderDecisionTable(
      ...
      [
        ...
        { key: "effective_sla_target_ticks", label: "SLA Target" },
        {
          key: "action",
          label: "Action",
          render: row => `<button data-open-sensitivity="${escapeHtml(row.study_id)}">Open</button>`
        }
      ],
      items
    );
...
async function openSensitivityDetail(studyId) {
  ...
  try {
    const detailData = await decisionFetchJson(
      `/api/decision/sensitivity/${encodeURIComponent(studyId)}`
    );

    let tornadoData = { tornado: {} };

    try {
      tornadoData = await decisionFetchJson(
        `/api/decision/sensitivity/${encodeURIComponent(studyId)}/tornado`
      );
    } catch (error) {
    ...
    el.innerHTML = `
      <div class="decision-toolbar">
        <button data-back-sensitivity>Back to sensitivity studies</button>
      </div>
      ...
      <h3>Baseline</h3>
      <div class="sens-baseline-table"></div>

      <h3>Factor Results</h3>
      <div class="sens-factor-table"></div>
    ...
    if (toNumber(summary.failed_runs, 0) > 0) {
      renderErrorsTable(el.querySelector(".sens-errors-table"), errors);
    }

    renderRowsTable(
      el.querySelector(".sens-baseline-table"),
      [baseline],
      {
        flatten: true,
        preferredKeys: [
    ...
    renderRowsTable(
      el.querySelector(".sens-factor-table"),
      factorResults,
      {
        flatten: true,
        linkRunId: true,
    ...
    if (!tornadoEntries.length) {
      ...
      tornadoEntries.forEach(([target, rows]) => {
        ...
        block.innerHTML = `
          ...
          <div class="sens-tornado-target"></div>
        `;
        ...
        renderRowsTable(
          ...
          {
            flatten: true,
            preferredKeys: ["factor", "name", "value", "delta", "delta_percent"]
          }
        );
      });
    }
    ...
    renderPaginatedDynamicTable(
      el.querySelector(".sens-results-table"),
      (page, pageSize) =>
        `/api/decision/sensitivity/${encodeURIComponent(studyId)}/results?page=${page}&page_size=${pageSize}`,
      {
        flatten: true,
        linkRunId: true,
        preferredKeys: [
...
async function loadDecisionOptimizations() {
  ...
  try {
    ...
    if (!items.length) {
      setDecisionEmpty(el, "No optimization studies found.");
      return;
    }
    ...
    renderDecisionTable(
      el,
      [
        { key: "opt_id", label: "Optimization ID" },
        { key: "generated_at", label: "Generated" },
        { key: "objective", label: "Objective" },
        { key: "direction", label: "Direction" },
        { key: "n_trials", label: "Trials" },
        { key: "feasible_count", label: "Feasible" },
        { key: "infeasible_count", label: "Infeasible" },
        {
          key: "action",
          label: "Action",
          render: row => `<button data-open-optimization="${escapeHtml(row.opt_id)}">Open</button>`
        }
      ],
      items
    );
...
async function openOptimizationDetail(optId) {
  ...
  try {
    const data = await decisionFetchJson(
      `/api/decision/optimizations/${encodeURIComponent(optId)}`
    );
    ...
    el.innerHTML = `
      <div class="decision-toolbar">
        <button data-back-optimization>Back to optimization studies</button>
      </div>
      ...
      <div class="decision-status warning">
        Single-replica results are exploratory. Verify with robustness.
      </div>
      ...
      <h3>Constraints</h3>
      <div class="opt-constraints-table"></div>

      <h3>Search Space</h3>
      <div class="opt-search-space-table"></div>
    ...
    if (bestTrial) {
      renderRowsTable(
        el.querySelector(".opt-best-trial"),
        [bestTrial],
        {
          flatten: true,
          preferredKeys: ["number", "state", "feasible", "value"]
        }
      );
    }

    renderObjectTable(
      el.querySelector(".opt-constraints-table"),
      summary.constraints || {},
      "Constraint",
      "Limit"
    );
    ...
    renderRowsTable(
      el.querySelector(".opt-search-space-table"),
      summary.search_space || [],
      {
        flatten: true,
        preferredKeys: ["name", "type", "low", "high", "choices"]
      }
    );

    renderPaginatedDynamicTable(
      el.querySelector(".opt-trials-table"),
      (page, pageSize) =>
        `/api/decision/optimizations/${encodeURIComponent(optId)}/trials?page=${page}&page_size=${pageSize}`,
      {
        flatten: true,
        linkRunId: true,
        preferredKeys: ["number", "state", "feasible", "value", "objective"]
      }
    );
...
async function loadDecisionMultiobjective() {
  ...
  try {
    ...
    if (!items.length) {
      setDecisionEmpty(el, "No multi-objective studies found.");
      return;
    }
    ...
    renderDecisionTable(
      ...
      [
        ...
        { key: "pareto_count", label: "Pareto" },
        {
          key: "action",
          label: "Action",
          render: row => `<button data-open-multiobjective="${escapeHtml(row.study_id)}">Open</button>`
        }
      ],
      items
    );
...
async function openMultiobjectiveDetail(studyId) {
  ...
  try {
    const data = await decisionFetchJson(
      `/api/decision/multiobjective/${encodeURIComponent(studyId)}`
    );
    ...
    el.innerHTML = `
      <div class="decision-toolbar">
        <button data-back-multiobjective>Back to multi-objective studies</button>
      </div>
      ...
      <h3>Objectives</h3>
      <div class="mo-objectives-table"></div>

      <h3>Constraints</h3>
      <div class="mo-constraints-table"></div>

      <h3>Search Space</h3>
      <div class="mo-search-space-table"></div>
    ...
    renderRowsTable(
      el.querySelector(".mo-objectives-table"),
      summary.objectives || [],
      {
        flatten: true,
        preferredKeys: ["metric", "direction"]
      }
    );

    renderObjectTable(
      el.querySelector(".mo-constraints-table"),
      summary.constraints || {},
      "Constraint",
      "Limit"
    );
    ...
    renderRowsTable(
      el.querySelector(".mo-search-space-table"),
      summary.search_space || [],
      {
        flatten: true,
        preferredKeys: ["name", "type", "low", "high", "choices"]
    ...
    renderPaginatedDynamicTable(
      el.querySelector(".mo-pareto-table"),
      (page, pageSize) =>
        `/api/decision/multiobjective/${encodeURIComponent(studyId)}/pareto?page=${page}&page_size=${pageSize}`,
      {
        flatten: true,
        preferredKeys: [
          "trial_number",
    ...
    renderPaginatedDynamicTable(
      el.querySelector(".mo-trials-table"),
      (page, pageSize) =>
        `/api/decision/multiobjective/${encodeURIComponent(studyId)}/trials?page=${page}&page_size=${pageSize}`,
      {
        flatten: true,
        linkRunId: true,
        preferredKeys: ["number", "state", "feasible"]
      }
    );
...
async function loadDecisionRobustness() {
  ...
  try {
    ...
    if (!items.length) {
      setDecisionEmpty(el, "No robustness studies found.");
      return;
    }
    ...
    renderDecisionTable(
      el,
      [
        { key: "robust_id", label: "Robustness ID" },
        { key: "generated_at", label: "Generated" },
        { key: "reps", label: "Reps" },
        { key: "objective", label: "Objective" },
        { key: "direction", label: "Direction" },
        ...
        { key: "failed_runs", label: "Failed" },
        {
          ...
          render: row =>
            row.recommended == null
              ? "-"
              : escapeHtml(
                  typeof row.recommended === "object"
                    ? row.recommended.label || JSON.stringify(row.recommended)
                    : row.recommended
                )
        },
        {
          key: "action",
          label: "Action",
          render: row => `<button data-open-robustness="${escapeHtml(row.robust_id)}">Open</button>`
        }
      ],
      items
    );
...
async function openRobustnessDetail(robustId) {
  ...
  try {
    const data = await decisionFetchJson(
      `/api/decision/robustness/${encodeURIComponent(robustId)}`
    );
    ...
    el.innerHTML = `
      <div class="decision-toolbar">
        <button data-back-robustness>Back to robustness studies</button>
      </div>
      ...
      ${
        summary.recommended == null
          ...
          : `<div class="decision-status success">Recommended: ${escapeHtml(
              typeof summary.recommended === "object"
                ? summary.recommended.label || JSON.stringify(summary.recommended)
                : summary.recommended
            )}</div>`
      }
      ...
      <h3>Constraints</h3>
      <div class="rob-constraints-table"></div>
    ...
    if (toNumber(summary.failed_runs, 0) > 0) {
      renderErrorsTable(el.querySelector(".rob-errors-table"), errors);
    }

    renderObjectTable(
      el.querySelector(".rob-constraints-table"),
      summary.constraints || {},
      "Constraint",
      "Limit"
    );
    ...
    if (!candidates.length) {
      ...
      renderDecisionTable(
        ...
        [
          ...
          {
            key: "constraint_pass_probability",
            label: "Pass Probability",
            render: row => probabilityBarHtml(row.constraint_pass_probability, minPassProbability)
          },
          {
            key: "robust_pass",
            label: "Robust Pass",
            render: row => robustPassBadge(row.robust_pass)
          },
          { key: "objective_mean", label: "Objective Mean" },
          { key: "objective_stdev", label: "Objective Stdev" },
          {
            key: "constraint_details",
            label: "Details",
            render: row => `
              <details>
                <summary>Details</summary>
                <pre class="json-preview">${escapeHtml(JSON.stringify(row.constraint_details || {}, null, 2))}</pre>
              </details>
            `
          }
        ],
    ...
    renderPaginatedDynamicTable(
      el.querySelector(".rob-results-table"),
      (page, pageSize) =>
        `/api/decision/robustness/${encodeURIComponent(robustId)}/results?page=${page}&page_size=${pageSize}`,
      {
        flatten: true,
        linkRunId: true,
        preferredKeys: [
    ...
    el.onclick = event => {
      if (event.target.closest("[data-back-robustness]")) {
...
async function loadDecisionJobs() {
  ...
  try {
    ...
    if (hasActive && !jobPollingInterval) {
      startJobPolling();
    } else if (!hasActive && jobPollingInterval) {
      stopJobPolling();
    }
  } catch (error) {
    setDecisionError(el, error.message);
...
function renderJobs(el, jobs) {
      ...
      { key: "artifact_type", label: "Artifact Type" },
      {
        ...
        render: row => {
          if (row.status === "finished" && row.artifact_type && row.artifact_id) {
            return `
              <button
                data-open-artifact-type="${escapeHtml(row.artifact_type)}"
                data-open-artifact-id="${escapeHtml(row.artifact_id)}"
              >
                Open
              </button>
            `;
          ...
          if (row.status === "failed" && row.error) {
            return `
              <details>
                ...
                <pre class="json-preview">${escapeHtml(row.error)}</pre>
              </details>
            `;
          }
  ...
  el.onclick = event => {
    ...
    if (openBtn) {
      openArtifact(openBtn.dataset.openArtifactType, openBtn.dataset.openArtifactId);
      return;
    }
...
function openArtifact(type, id) {
  ...
  if (!entry) {
    alert(`Opening artifact type '${type}' is not supported yet.`);
    return;
  }
...
async function cancelJob(jobId) {
  ...
  try {
    const res = await fetch(
      `/api/decision/jobs/${encodeURIComponent(jobId)}/cancel`,
      { method: "POST" }
    );
...
function startJobPolling() {
  ...
  jobPollingInterval = setInterval(() => {
    if (currentView === "decision" && decisionSubview === "jobs") {
      loadDecisionJobs();
    }
  }, 2000);
}
...
function loadJobTemplate(module) {
  ...
  const costConfigTemplate = JSON.parse(JSON.stringify(DEFAULT_EXAMPLE_COST_CONFIG));
  ...
  const templates = {
    monte_carlo: {
      ...
      base_seed: 42,
      sla_target_ticks: 300,
      cost_config: costConfigTemplate
    },
    ...
    optimizer: {
      ...
      reps: 1,
      base_seed: 42,
      cost_config: JSON.parse(JSON.stringify(DEFAULT_EXAMPLE_COST_CONFIG)),
      constraints: {
